...
void loop() {
  ...
  for(int y = 0; y < yMax; y++){
    for(int x = 0; x < xMax; x++){
      for(int z = 0; z < NUM_LEDS; z++){
         leds[z].r = pgm_read_byte_near(special + x*(3*NUM_LEDS) + (z*3));
         leds[z].g = pgm_read_byte_near(special + x*(3*NUM_LEDS) + (z*3) + 1);
         leds[z].b = pgm_read_byte_near(special + x*(3*NUM_LEDS) + (z*3) + 2);
      }
      //image7;
      FastLED.show();
      /*delay(5);
...
/*
  ...
  for(int y = 0; y < yMax; y++){
    for(int x = 0; x < xMax; x++){
      for(int z = 0; z < NUM_LEDS; z++){
         leds[z].r = pgm_read_byte_near(image1 + x*(3*NUM_LEDS) + (z*3));
         leds[z].g = pgm_read_byte_near(image1 + x*(3*NUM_LEDS) + (z*3) + 1);
         leds[z].b = pgm_read_byte_near(image1 + x*(3*NUM_LEDS) + (z*3) + 2);
      }
      //image7;
      FastLED.show();
      Serial.println(x);
